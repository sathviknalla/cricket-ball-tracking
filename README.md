...
- **Segmented Trajectory Modeling:**
  ...
  2. *Bounce Inflection:* Coefficient of restitution ($e \in [0.55, 0.75]$) with spin angle deflection.
  3. *Post-Impact Projection to Stumps ($Y=20.12\text{ m}$):* Numerical forward simulation considering aerodynamic drag ($\gamma$) to predict 3D impact at the wicket line.
...
### 6. Synthetic Training Pipeline (`training/`)
- `training/dataset.py`: Synthetic broadcast generator using Albumentations for angled motion-blurred streaks (>140 km/h).
- `training/train.py`: Ultralytics YOLOv8 fine-tuning script with small-object focus (`imgsz=1280`, focal loss).
...
pytest tests/ -v
```
All deterministic unit tests pass with zero external video dependencies.
